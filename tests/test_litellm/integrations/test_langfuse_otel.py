...
class TestLangfuseOtelIntegration:
    ...
    def test_streaming_response_model_uses_proxy_alias(self):
        ...
        assert response_obj.model == upstream_model
        assert attrs.get("langfuse.observation.model.name") == proxy_alias
        assert attrs["langfuse.observation.model.name"] != upstream_model

        stamp_index = next(
            i for i, call in enumerate(mock_span.set_attribute.call_args_list)
            if call.args[0] == "langfuse.observation.model.name" and call.args[1] == proxy_alias
        )
        response_model_indices = [
            i for i, call in enumerate(mock_span.set_attribute.call_args_list)
            if call.args[0] == "llm.response.model"
        ]
        assert response_model_indices, "arize helper did not stamp llm.response.model"
        assert stamp_index > max(response_model_indices)
